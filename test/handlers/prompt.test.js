...
describe('handlers/prompt', function() {
  ...
  describe('handler', function() {
    ...
    it('should next with error when password store cannot be created', function(done) {
      ...
      container.create = sinon.stub()
      container.create.withArgs('module:@authnomicon/login.IdentifierRouter').rejects(noIdentifierRouterErr);
      var failPasswordStoreErr = new Error("Unable to create component 'app/foo' required by 'app/passwordstore'");
      failPasswordStoreErr.code = 'IMPLEMENTATION_NOT_FOUND';
      failPasswordStoreErr.interface = 'app/foo';
      container.create.withArgs('module:@authnomicon/credentials.PasswordStore').rejects(failPasswordStoreErr);
      ...
      chai.express.use(handler)
        ...
        .next(function(err) {
          ...
          expect(err).to.be.an.instanceOf(Error);
          expect(err.message).to.equal("Unable to create component 'app/foo' required by 'app/passwordstore'");
          done();
        })
        .listen();
    }); // should next with error when password store cannot be created
